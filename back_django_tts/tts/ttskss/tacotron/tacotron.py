...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, num_speakers, speaker_id,mel_targets=None, linear_targets=None, loss_coeff=None,
        ...
        with tf.variable_scope('inference', reuse=tf.AUTO_REUSE) as scope:
            ...
            # scope.reuse_variables()
            # char_embed_table2 = tf.get_variable('embedding', [len(symbols), hp.embedding_size], dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.5))
            # assert char_embed_table2 == char_embed_table
            zero_pad = True
            ...
            # print("셀즈", cells)
            # print("디코더 셀", decoder_cell)
            # print("아웃풋 셀 뱃치 사이즈", output_cell.zero_state(batch_size=batch_size, dtype=tf.float32))
            # print("아웃풋 셀", output_cell)
            ...
            if hp.model_type == "deepvoice":
                ...
                print("디코더 알엔엔 이닛", decoder_rnn_init_states)
