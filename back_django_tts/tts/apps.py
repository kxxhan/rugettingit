# coding: utf-8
"""
 python synthesizer.py --load_path logdir-tacotron/kss+moon_2021-09-28_10-49-34 --num_speakers 2 --speaker_id 0 --text "안녕하세요 여러분 저는 점심으로 간장계란밥을 먹을거에요."
"""
from django.apps import AppConfig
# from ttskss.synthesizer import Synthesizer
...
from .ttskss.hparams import hparams
from .ttskss.tacotron import create_model, get_most_recent_checkpoint
from .ttskss.utils.audio import save_wav, inv_linear_spectrogram, inv_preemphasis, inv_spectrogram_tensorflow
from .ttskss.utils import plot, PARAMS_NAME, load_json, load_hparams, add_prefix, add_postfix, get_time, parallel_run, makedirs, str2bool
from .ttskss.text.korean import tokenize
from .ttskss.text import text_to_sequence, sequence_to_text
# from .ttskss.datasets.datafeeder_tacotron import _prepare_inputs
import warnings
...
def plot_graph_and_save_audio(args,
    ...
    print("@ base_path setting")
    if base_path:
        file_name = get_time()
        plot_path = "{}/{}.png".format(base_path, file_name)
        trans_file_name(file_name)
    elif path:
...
if __name__ == "__main__":
    ...
    synthesizer.synthesize(texts=[config.text],base_path=config.sample_path,speaker_ids=[config.speaker_id],
